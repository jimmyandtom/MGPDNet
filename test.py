...
def main(_run, _config, _log):
    ...
    valid_classes = ['LIVER', 'RK', 'LK', 'SPLEEN']
    labels = {k:v for k,v in get_label_names(_config['dataset']).items() if v in valid_classes}
    class_dice = {}
    class_iou = {}

    _log.info(f'Starting validation...')
    for label_val, label_name in labels.items():
        if label_name == 'BG' or (not np.intersect1d([label_val], _config['test_label'])):
            continue
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(test_loader):  # this "for" loops 4 times
                ...
                for sub_chunck in range(_config['n_part']):  # n_part = 3
                    ...
                    query_label_s = query_label[0][idx_[sub_chunck]:idx_[sub_chunck + 1]]# C' x 3 x H x W
                    query_pred_s = []
                    for i in range(query_image_s.shape[0]):
                        _pred_s, _, _, _, _ = model([support_image_s], [support_fg_mask_s], [query_image_s[[i]]], [query_label_s[[i]]], train=False)  # 1 x 2 x H x W  # 1 x 2 x H x W
                ...
                _log.info(f'Tested query volume: {sample["id"][0][len(_config["path"][_config["dataset"]]["data_dir"]):]}.')
